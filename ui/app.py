# ui/app.py — main window, sidebar nav, content frame
...
from ui.initial_matrix_panel import InitialMatrixPanel
...
class DFTApp(tk.Tk):
    ...
    def _build_sidebar(self):
        ...
        logo.pack(fill="x")
        tk.Label(logo, text="◈", font=("Segoe UI", 32),
                 bg=BG_CARD, fg=ACCENT_TEAL).pack()
        ...
        self.nav_btns = {}
        items = [
            ("home",   "⌂",  "Home",        ACCENT_TEAL),
            ("solver", "⚙",  "DFT Solver",  ACCENT_TEAL),
            ("idft",   "↩",  "IDFT Solver", ACCENT_PURP),
            ("about",  "ℹ",  "About",       ACCENT_TEAL),
        ]
        for key, icon, label, color in items:
            self.nav_btns[key] = self._nav_button(key, icon, label, color)

        tk.Frame(self.sidebar, bg=BORDER, height=1).pack(fill="x", padx=16, pady=6)
        tk.Label(self.sidebar, text="v1.1  ·  NIT Jalandhar",
                 font=FONT_SMALL, bg=BG_CARD, fg=TEXT_MUTED).pack(side="bottom", pady=16)

    def _nav_button(self, key, icon, label, active_color=ACCENT_TEAL):
        ...
        def on_click(_=None): self._nav(key)
        def on_enter(_):
            if self._active != key:
    ...
    def _nav(self, key):
        ...
        for k, frame in self.nav_btns.items():
            ...
            fg = frame._active_color if active else TEXT_MUTED
        ...
        elif key == "solver": self._show_solver()
        elif key == "idft":   self._show_idft()
        elif key == "about":  self._show_about()
    ...
    def _show_solver(self):
        ...
        style.configure("DFT.TNotebook",  background=BG_DARK, borderwidth=0)
        ...
        def on_result(signal, X, W_mat, N):
            ...
            self.w_matrix    = W_mat
            self.dft_N       = N
            self._init_panel.render(signal, N)           # ❓ question
            self._matrix_panel.render(W_mat, N)          # 🔢 answer
            self._output_panel.render(signal, X, W_mat, N)
            self._plot_panel.render(signal, X, N)
            nb.select(tab_init)                          # open on question tab

        InputPanel(left, on_result_callback=on_result).pack(fill="both", expand=True)
    def _show_idft(self):
        from ui.idft_panel import IDFTSolverPage

        hdr = tk.Frame(self.content, bg=BG_CARD, pady=14, padx=24)
        hdr.pack(fill="x")
        tk.Label(hdr, text="↩  Inverse DFT Solver",
                 font=("Segoe UI", 16, "bold"),
                 bg=BG_CARD, fg=ACCENT_PURP).pack(side="left")
        tk.Label(hdr,
                 text="  Reconstruct x[n] from frequency coefficients X[k]",
                 font=("Segoe UI", 11), bg=BG_CARD, fg=TEXT_MUTED).pack(side="left")

        IDFTSolverPage(self.content).pack(fill="both", expand=True)
